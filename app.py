...
SCHOOL_LEVEL_OPTIONS = ["1학년", "2학년", "3학년"]
...
def ensure_pdf_template_presets(db: sqlite3.Connection) -> None:
    ...
    for preset in PDF_TEMPLATE_PRESETS:
        ...
        db.execute(
            """
            INSERT INTO program_templates (name, description, questions_json, created_at)
            VALUES (?, ?, ?, ?)
            """,
            (
                preset["name"],
...
def seed_defaults(db: sqlite3.Connection) -> None:
    ...
    if not teacher_row:
        db.execute(
            """
            INSERT INTO teachers (name, email, access_code, academic_info, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            ("김강사", "teacher@example.com", "TCHR2026", "학교 신분\nKAIST 박사과정", now_iso()),
        )
        teacher_row = db.execute("SELECT * FROM teachers WHERE access_code = ?", ("TCHR2026",)).fetchone()
    ...
    if not db.execute("SELECT 1 FROM programs LIMIT 1").fetchone():
        ...
        db.execute(
            ...
            INSERT INTO programs (
                ...
                template_id, template_name, template_description, questions_json,
...
def generate_ai_evaluation_example(program: sqlite3.Row | dict[str, Any], submission: dict[str, Any]) -> tuple[str, str]:
    ...
    body = {
        ...
        "input": [
            {
                "role": "developer",
                "content": [{"type": "input_text", "text": EVALUATION_EXAMPLE_PROMPT}],
            },
            {
                "role": "user",
...
def landing(request: Request) -> Response:
    ...
    if not active_role and request.session:
...
def login_admin(request: Request) -> Response:
    ...
    if not admin or not verify_password(password, admin["password_hash"]):
        return redirect_response("/?role=admin&error=관리자%20ID%20또는%20비밀번호가%20올바르지%20않습니다.")
...
def login_teacher(request: Request) -> Response:
    ...
    if not teacher:
        return redirect_response("/?role=teacher&error=강사%20접속%20코드를%20확인해%20주세요.")
...
def login_student(request: Request) -> Response:
    program_code = re.sub(r"\D", "", request.form.get("program_code", "").strip())[:8]
    ...
    if not program:
        return redirect_response("/?role=student&error=유효한%208자리%20프로그램%20코드를%20입력해%20주세요.")
...
def admin_create_teacher(request: Request) -> Response:
    ...
    try:
        request.db.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (name, email, access_code, academic_info, now_iso()),
        )
        request.db.commit()
    except sqlite3.IntegrityError:
        set_flash(request.db, request.session["id"], "이미 사용 중인 강사 접속 코드입니다.", "error")
        return redirect_response("/admin")
    set_flash(request.db, request.session["id"], f"{name} 강사가 등록되었습니다. 접속 코드는 {access_code} 입니다.", "success")
    return redirect_response("/admin")
...
def admin_create_template(request: Request) -> Response:
    ...
    try:
        request.db.execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (name, description, json_dump(questions), now_iso()),
        )
        request.db.commit()
    except sqlite3.IntegrityError:
...
def student_submit(request: Request) -> Response:
    ...
    if saved_submission_id and openai_is_configured():
        ...
        target = next((item for item in normalized_submission if item["id"] == saved_submission_id), None)
        if target:
            ensure_ai_suggestion_for_submission(request.db, program, target, force=True)
    set_flash(request.db, request.session["id"], "학생 제출이 완료되었습니다. 강사가 확인할 수 있습니다.", "success")
